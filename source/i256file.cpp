//
// C++ Encoder/Decoder
// For I256 Foenix Picture File Format
// 
...
// your compiler
static_assert(sizeof(I256File_Header)==16, "I256File_Header is supposed to be 16 bytes");
static_assert(sizeof(I256File_CLUT)==10,  "I256File_CLUT is supposed to be 10 bytes");
static_assert(sizeof(I256File_PIXL)==10, "I256File_PIXL is supposed to be 10 bytes");
static_assert(sizeof(I256File_TMAP)==14, "I256File_TMAP is supposed to be 14 bytes");
static_assert(sizeof(I256File_CHUNK)==8, "I256File_CHUNK is supposed to be 8 bytes");
...
//------------------------------------------------------------------------------
// Create a blank I256File constructor
//
I256File::I256File(int iWidthPixels, int iHeightPixels, int iNumColors)
...
I256File::~I256File()
{
	...
	for (int idx = 0; idx < m_pTileMaps.size(); ++idx)
	{
		delete[] m_pTileMaps[idx];
		m_pTileMaps[ idx ] = nullptr;
	}
...
void I256File::SetPalette( const I256_Palette& palette )
{
	// copy in the colors
...
//
void I256File::AddImages( const std::vector<unsigned char*>& pPixelMaps )
{
...
// Make a Copy of the image data
//
void I256File::AddTileMaps( const std::vector<unsigned short*>& pTileMaps, unsigned short width, unsigned short height )
{
	m_widthTiles = width;
	m_heightTiles = height;

	int numNames = width * height;

	for (int idx = 0; idx < pTileMaps.size(); ++idx)
	{
		unsigned short* pTileMap = new unsigned short[ numNames ];
		memcpy(pTileMap, pTileMaps[ idx ], numNames*sizeof(unsigned short));
		m_pTileMaps.push_back( pTileMap );
	}
}
...
void I256File::SaveToFile(const char* pFilenamePath)
{
	...
	//--------------------------------------------------------------------------
	// Add the header
	bytes.resize( bytes.size() + sizeof(I256File_Header) );
	...
	I256File_Header* pHeader = (I256File_Header*)&bytes[0];
	...
	//--------------------------------------------------------------------------
	// Add a CLUT Chunk
	unsigned int clut_size = (m_pal.iNumColors * 4) + sizeof(I256File_CLUT);
	...
	{
		// Save compressed
		clut_size = (unsigned int) (compSize + sizeof(I256File_CLUT));
		// Add space for the CLUT
		bytes.resize( bytes.size() + clut_size );
		I256File_CLUT* pCLUT = (I256File_CLUT*) &bytes[ clut_offset ];
		...
		memcpy(&bytes[ clut_offset + sizeof(I256File_CLUT) ], pCompressedBuffer,
			   compSize);
	...
	bytes.resize( bytes.size() + sizeof(I256File_PIXL) );
	I256File_PIXL* pPIXL = (I256File_PIXL*)&bytes[ pixl_offset ];
	...
	pPIXL = (I256File_PIXL*)&bytes[ pixl_offset ];
	pPIXL->chunk_length = (unsigned int) (bytes.size() - pixl_offset);

	//--------------------------------------------------------------------------
	// Add a TMAP Chunk, which has the map name data
	...
	//--------------------------------------------------------------------------
	// Update the header
	pHeader = (I256File_Header*)&bytes[0]; // Required
	pHeader->file_length = (unsigned int)bytes.size(); // get some valid data in there
...
void I256File::LoadFromFile(const char* pFilePath)
{
	...
	if (bytes.size())
	{
		...
		file_offset += sizeof(I256File_Header);
		...
		while (file_offset < bytes.size())
		{
			...
			I256File_CLUT* pCLUT = (I256File_CLUT*)&bytes[ file_offset ];
			I256File_PIXL* pPIXL = (I256File_PIXL*)&bytes[ file_offset ];
			I256File_TMAP* pTMAP = (I256File_TMAP*)&bytes[ file_offset ];
			I256File_CHUNK* pCHUNK = (I256File_CHUNK*)&bytes[ file_offset ];
...
//
void I256File::UnpackClut(I256File_CLUT* pCLUT)
{
	...
	unsigned char* pBGRA = ((unsigned char*) pCLUT) + sizeof(I256File_CLUT);

	m_pal.pColors = new I256_Color[ numColors ];
	...
	if (pCLUT->num_colors & 0x8000)
	{
		...
		lzsa_decompress_inmem(pBGRA, 		   // Compressed Data
							  ...
							  pCLUT->chunk_length-sizeof(I256File_CLUT),  // compressed size in bytes
							  numColors * 4,
							  LZSA_FLAG_RAW_BLOCK,
							  &version);
...
//
void I256File::UnpackPixel(I256File_PIXL* pPIXL)
{
	int num_blobs = pPIXL->num_blobs;

	unsigned char *pData = ((unsigned char*)pPIXL) + sizeof(I256File_PIXL);
...
//------------------------------------------------------------------------------
//
// Unpack the tilemap, that's been weirdly packed into 64KB chunks
// to make it easier to deal with on 65816
//
void I256File::UnpackTileMap(I256File_TMAP* pTMAP)
{
	int num_blobs = pTMAP->num_blobs;

	unsigned char *pData = ((unsigned char*)pTMAP) + sizeof(I256File_TMAP);

	m_widthTiles  = pTMAP->width;
	m_heightTiles = pTMAP->height;

	size_t bufferSize = m_widthTiles * m_heightTiles * sizeof(unsigned short);

	unsigned char* pTargetBuffer = new unsigned char[ bufferSize ];
	m_pTileMaps.push_back((unsigned short*)pTargetBuffer);
